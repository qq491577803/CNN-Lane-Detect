...
def recongise_light(frame):
        frame = cv2.resize(frame,(160,90))
        cv2.imshow("orimage",frame)
	red_val = 0
	green_val = 65
	hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
	red_lower = np.array([red_val-10,100,100])
	red_upper = np.array([red_val+10,255,255])
	green_lower = np.array([green_val-30,100,100])
	green_upper = np.array([green_val+30,255,255])
	...
	print('*-*-*-*-*-Mask_Arear :',green_black)
	if 800>green_black > 50:
		GreenLight = 1
		print '*-*-*-*-*-Green Light -**-*-*-*-*-*-*-*-'
	else:

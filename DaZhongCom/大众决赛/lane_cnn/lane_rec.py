...
def train(epochs=4000):
    ...
    x = tf.placeholder(tf.float32, shape=[None, 180, 320, 3],name = 'x_input')
    y = tf.placeholder(tf.float32, shape=[None, 180, 320, 1],name = 'y_input')
...
def prediction():
    ...
    with tf.Session() as sess:
        ...
        loader = tf.train.import_meta_graph('./model_test/lane-conv-127.meta')
        loader.restore(sess, './model_test/lane-conv-127')
        ...
        for fp in fpaths:
            ...
            imgarr = np.reshape(img,newshape=[1,180,320,3])

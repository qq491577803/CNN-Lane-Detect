...
#     return filename
def read_data():
    train_fpth = './training/train'
    f_num =  len(glob(train_fpth + '*.jpg'))
    X = []
    for i in range(f_num):
        fpth = train_fpth+str(i+1) + '.jpg'
        arr = cv2.imread(fpth)
        arr = cv2.resize(arr,dsize=(160,90))
        X.append(arr)
    X = np.stack(X)
    # Y = [[37,118,1,124,68,2,195,80,3],[39,62,1,143,68,2,223,101,3],[92,144,3,0,0,0,0,0,0],[103,36,1,0,0,0,0,0,0],[102,134,3,0,0,0,0,0,0],[39,63,3,223,101,3,142,67,2],[241,96,3,0,0,0,0,0,0],[258,103,1,0,0,0,0,0,0],[109,141,1,0,0,0,0,0,0],[47,50,1,108,141,1,240,95,3],[23,52,2,0,0,0,0,0,0]]
    Y_r = [[37,118,124,68,195,80],[39,62,143,68,223,101],[92,144,0,0,0,0],[103,36,0,0,0,0],[102,134,0,0,0,0],[39,63,223,101,142,67],[241,96,0,0,0,0],[258,103,0,0,0,0],[109,141,0,0,0,0],[47,50,108,141,240,95],[23,52,0,0,0,0]]
    Y_c = [[1,2,3],[1,2,3],[3,0,0],[1,0,0],[3,0,0],[3,3,2],[3,0,0],[1,0,0],[1,0,0],[1,1,3],[2,0,0]]
    Y_r = np.array(Y_r)
    Y_c = np.array(Y_c)
    # Y = {'Y_c':Y_c,'Y_r':Y_r}
    data = {'x':X,'y_c':Y_c,'y_r':Y_r}
    return data
...
def train(epochs=4000):
    ...
    y_r = tf.placeholder(tf.float32, shape=[None, 6],name = 'y_r_input')
    y_c = tf.placeholder(tf.float32,shape=[None,3],name='y_c_input')
    ...
    with tf.name_scope('layers'):
        ...
        with tf.name_scope('layers2'):
            with tf.name_scope('W2'):
                W_conv2 = wight_varible([3, 3, 20, 10])
        ...
        with tf.name_scope('layers3'):
            with tf.name_scope('W3'):
                W_conv3 = wight_varible([5, 5, 10, 5])
        ...
        with tf.name_scope('layer7'):
            with tf.name_scope('W7'):
                W_conv7 = wight_varible([5, 5, 5, 3])
            with tf.name_scope('b7'):
                b_conv7 = bias_variable([3])
            with tf.name_scope('relu7'):
                h_conv7 = tf.nn.relu(conv2d(h3_pool, W_conv7) + b_conv7)  # (?,123,440,1)
        h7_pool = max_pool_2x2(h_conv7)
    #全连接层
        #坐标回归
        flat = tf.reshape(h7_pool, shape=[-1, 23*40*3])
        W_fc1 = wight_varible([23*40*3, 1024])
        b_fc1 = bias_variable([1024])
        h_fc1 = tf.nn.relu(tf.matmul(flat,W_fc1)+b_fc1)
        W_fc2 = wight_varible([1024,6])
        b_fc2 = bias_variable([6])
        r_predict = (tf.matmul(h_fc1, W_fc2) + b_fc2)
        loss_r = tf.reduce_mean(tf.square(y_r - r_predict))
        #分类
        flat = tf.reshape(h7_pool, shape=[-1, 23 * 40 * 3])
        W_fc1 = wight_varible([23 * 40 * 3, 512])
        b_fc1 = bias_variable([512])
        h_fc1 = tf.nn.relu(tf.matmul(flat, W_fc1) + b_fc1)
        W_fc2 = wight_varible([512, 3])
        b_fc2 = bias_variable([3])
        c_predict = tf.nn.relu(tf.matmul(h_fc1, W_fc2) + b_fc2)

        loss_c = tf.reduce_mean(tf.square(y_c - c_predict))
        # loss_c = tf.reduce_mean(tf.nn.weighted_cross_entropy_with_logits(targets=y_c, logits=c_predict, pos_weight=50))
        loss = loss_r + loss_c
        optimis = tf.train.AdamOptimizer().minimize(loss)

        # loss = tf.reduce_mean(tf.nn.weighted_cross_entropy_with_logits(targets=y, logits=prediction, pos_weight=50))
        ...
        with tf.Session() as sess:
            ...
            for epoch in range(epochs):
                ...
                # predict = sess.run(prediction,feed_dict={x: data['x'], y: data['y'],prob:1})
                # print('Epoch', epoch, ' Loss:', loss_val)
                # # print("Predict:",predict)
                # if (epoch + 1) % 20== 0:
                #     saver.save(sess, "./model/lane-conv", global_step=epoch)
                optimiser = sess.run(optimis,feed_dict={x: data['x'], y_c:data['y_c'],y_r: data['y_r'],prob:0.7})
                loss_val = sess.run(loss,feed_dict={x: data['x'], y_c:data['y_c'],y_r: data['y_r'],prob:1})
                print('Epoch', epoch, ' Loss:', loss_val)
                if (epoch + 1) % 20== 0:
                    saver.save(sess, "./model/lane-conv", global_step=epoch)
                    c_prediction  = sess.run(c_predict,feed_dict={x: data['x'],prob:1})
                    r_prediction     = sess.run(r_predict,feed_dict={x: data['x'],prob:1})
                    print('c_predict:',c_prediction,'  r_predict:',r_prediction)
...
def prediction():
    with tf.Session() as sess:
        ...
        for f in fs:
            arr = cv2.imread(f)
            arr = cv2.resize(arr, (160, 90))
            imgarr = np.reshape(arr,newshape=[1,90,160,3])
            start = time.time()
            predict = sess.run(y, feed_dict={input_x: imgarr, keep_prob: 1.0})
            print(time.time() - start)
            predict_img = (predict[0]*255)
            predict_img = cv2.resize(predict_img,(640,360))
            cv2.imshow('pred',predict_img)
            cv2.waitKey(0)
        return predict_img
...
if __name__ == "__main__" :
    ...
    # with tf.Session() as sess:
    #     sess.run(tf.global_variables_initializer())
    #     loader = tf.train.import_meta_graph('./model/lane-conv-999.meta')
    #     loader.restore(sess, './model/lane-conv-999')
    #     y = tf.get_collection('pred_network')[0]
    #     graph = tf.get_default_graph()
    #     input_x = graph.get_operation_by_name('x_input').outputs[0]
    #     keep_prob = graph.get_operation_by_name('prob').outputs[0]
